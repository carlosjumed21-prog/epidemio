...
if excel_subido:
    try:
        ...
        with st.spinner(f"Estructurando datos de '{pestana_seleccionada}'..."):
            ...
            componente_html = generar_interfaz_y_script(df_pestana, pestana_seleccionada, orientacion, tipo_hoja)
        
        st.write("") 
        st.success(f"✅ Matriz de datos de la pestaña **'{pestana_seleccionada}'** procesada correctamente.")
        st.components.v1.html(componente_html, height=500, scrolling=True)
            
    except Exception as e:
        st.error(f"❌ Ocurrió un error al mapear la estructura del Excel: {e}")
else:
