...
# Verificación del archivo compartido desde el cargador en main.py
if 'archivo_compartido' not in st.session_state:
    st.info("👈 Por favor, sube el archivo HTML en el apartado de 'Configuración' de la barra lateral.")
else:
    try:
        ...
        if st.button("🚀 GENERAR EXCEL", use_container_width=True, type="primary"):
            ...
            if not especialidades_finales:
                ...
                for p in pacs_detectados:
                    if p["esp_real"] in especialidades_finales:
                        ...
                        datos_excel.append({
                            ...
                            "ESPECIALIDAD": p["esp_real"], "CAMA": p["CAMA"], 
                            "REGISTRO": p["REG"], "PACIENTE": p["PAC"], 
                            "SEXO": p["SEXO"], "EDAD": p["EDAD"], 
                            "DIAGNOSTICO": p["DIAG"], "FECHA_INGRESO": p["ING"], 
                            "DIAS_ESTANCIA": dias
                        })
                ...
                if datos_excel:
                    ...
                    # Definición de marcos (Bordes delgados)
                    thin_border = Border(
                    ...
                    # Agregar Tabla Oficial de Excel
