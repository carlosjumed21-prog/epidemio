...
if archivo_iaas:
    ...
    if st.button("🚀 2. Procesar Datos Base"):
        try:
            ...
            # Convertir fechas (Columnas A, B, D, E, G)
            for i in [0, 1, 3, 4, 6]:
            ...
            # Detección de Mes en Columna H (index 7) - Forzamos String para evitar fallas
            def detectar_mes(valor):
                texto = str(valor).lower()
                for abr, nombre in MESES_MAP.items():
                    if abr in texto: return nombre
                return "Otro"
            
            df['Mes_Nombre'] = df.iloc[:, 7].apply(detectar_mes)
            
            st.session_state['df_procesado'] = df
            st.success("✅ Datos listos. Configura los filtros y genera tu gráfica.")
    ...
    if 'df_procesado' in st.session_state:
        ...
        if st.button("📊 Generar / Actualizar Gráfica"):
            ...
            if not df_f.empty:
                # --- GRÁFICA ---
                df_plot = df_f.copy()
                # Limpiar negativos para visualización
                ...
                if s_sel == "Todos":
                    ...
                    fig = px.bar(comp_df, x=df_p.columns[5], y=cols_estudio, barmode='group',
                                 title="Desempeño Comparativo de Sujetos",
                    ...
                    fig = px.bar(evol_df, x='Mes_Nombre', y=cols_estudio, barmode='group',
                                 title=f"Evolución Mensual del Sujeto {s_sel}",
                ...
                st.warning(f"No hay datos encontrados para Sujeto: {s_sel} en los meses seleccionados. Revisa que la Columna H contenga el mes correspondiente.")
