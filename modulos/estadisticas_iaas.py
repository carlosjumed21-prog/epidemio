...
st.markdown("### Análisis Estadístico IAAS")
...
if archivo_iaas:
    ...
    if 'df_procesado' in st.session_state:
        ...
        # Las 4 columnas principales para las gráficas
        ...
        with c2:
            st.write("Seleccionar Meses (Casillas):")
        ...
        if not df_f.empty:
            ...
            # CASO 1: TODOS LOS SUJETOS
            if s_sel == "Todos":
                # Eje X: Sujetos | Eje Y: Días | Barras: 4 Columnas
                ...
                fig = px.bar(comp_df, x=df_p.columns[5], y=cols_grafica, barmode='group',
                             title="Comparativa de Tiempos entre Sujetos (Anual)",
                             labels={df_p.columns[5]: 'Sujetos (ID)', 'value': 'Promedio de Días'},
                             text_auto='.1f', color_discrete_sequence=px.colors.qualitative.Vivid)
                fig.update_xaxes(type='category', categoryorder='array', categoryarray=opciones_s[1:])

            # CASO 2: INDIVIDUO / MESES SELECCIONADOS
            else:
                # Eje X: Meses | Eje Y: Días | Barras: 4 Columnas
                evol_df = df_plot.groupby('Mes_Nombre')[cols_grafica].mean().reindex(MESES_ORDEN).dropna(how='all').reset_index()
                fig = px.bar(evol_df, x='Mes_Nombre', y=cols_grafica, barmode='group',
                             title=f"Evolución Mensual - Sujeto {s_sel}",
                             labels={'Mes_Nombre': 'Meses', 'value': 'Promedio de Días'},
            ...
            st.write("### Indicadores en Tiempo Real")
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter', datetime_format='dd/mm/yyyy') as writer:
            ...
            # Columnas auxiliares ocultas (N a R) para SUBTOTAL
            ...
            for i, col_aux in enumerate(letras_aux):
                worksheet.write_formula(max_r + 1, 8 + i, f"=SUBTOTAL(101, {col_aux}2:{col_aux}{max_r + 1})", fmt_v)

            worksheet.conditional_format(1, 8, max_r, 12, {'type': 'cell', 'criteria': '<', 'value': 0, 'format': fmt_r})
            worksheet.set_column(0, 12, 18)

        st.download_button("📥 Descargar Reporte Final (A-M)", output.getvalue(), "Reporte_IAAS_Grafico.xlsx")
