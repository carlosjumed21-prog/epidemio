...
if 'archivo_compartido' not in st.session_state:
    ...
    try:
        ...
        if not pacs_detectados:
            ...
            # --- SECCIÓN DE SELECCIÓN (RECUADRO DE CASILLAS) ---
            servicios_encontrados = sorted(list(set([p["esp_real"] for p in pacs_detectados])))
            
            with st.container(border=True):
                st.markdown("### 🛠️ Selección de Servicios para Insumos")
                servicios_seleccionados = st.multiselect(
                    "Seleccione los servicios que desea incluir en el reporte y previsualizar:",
                    options=servicios_encontrados,
                    default=servicios_encontrados, # Por defecto todos marcados
                    placeholder="Busque o seleccione especialidades..."
                )

            # --- PREVISUALIZACIÓN UNIFICADA ---
            if servicios_seleccionados:
                st.markdown("### 👁️ Previsualización de Datos Seleccionados")
                
                # Filtrar pacientes según la selección del multiselect
                datos_filtrados = [p for p in pacs_detectados if p["esp_real"] in servicios_seleccionados]
                df_p = pd.DataFrame(datos_filtrados)
                
                # Aplicar lógica de precauciones e insumos
                ...
                # --- BOTÓN DE GENERACIÓN ---
                st.write("---")
                if st.button("🚀 GENERAR EXCEL DE INSUMOS SELECCIONADOS", use_container_width=True, type="primary"):
                    hoy = datetime.now()
                    ...
                    st.success(f"✅ Reporte generado para {len(servicios_seleccionados)} servicios.")
                    st.download_button(
                ...
                st.info("Seleccione al menos una especialidad en el recuadro superior para ver la previsualización.")
